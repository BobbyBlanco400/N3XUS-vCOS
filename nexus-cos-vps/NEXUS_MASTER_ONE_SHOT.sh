...
# Colors (RED, GREEN, YELLOW, BLUE, NC are inherited from lib/nginx-safe-deploy.sh)
MAGENTA='\033[0;35m'
CYAN='\033[0;36m'
...
log_info "Using local deployment package from PR #199..."
mkdir -p "${NEXUS_REPO_PATH}"
# Copy all files from current directory to target, overwriting
cp -R . "${NEXUS_REPO_PATH}/"
cd "${NEXUS_REPO_PATH}"
...
if [ -f ".env.production" ]; then
    log_info "Using production environment variables..."
    cp .env.production .env
fi

log_success "Repository files installed to ${NEXUS_REPO_PATH}"
...
# ---------------------------
log_step "STEP 6/12: Docker Stack Deployment"
...
# HEALTH CHECKS
# ---------------------------
log_step "STEP 7/12: Health Check Validation (120s window)"
...
# ---------------------------
log_step "STEP 8/12: Nexus-Handshake 55-45-17 Compliance"
