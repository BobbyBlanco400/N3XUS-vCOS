...
**Module Name**: puabo-ott-tv-streaming
**Purpose**: OTT streaming
...
### Installation
```bash
npm install @nexus-cos/puabo-ott-tv-streaming
```

### Basic Usage
```javascript
import { puabo-ott-tv-streaming } from '@nexus-cos/puabo-ott-tv-streaming';

// Initialize module
const module = new puabo-ott-tv-streaming(config);
...
```bash
cd modules/puabo-ott-tv-streaming
npm install
npm run dev
```
...
```bash
DEBUG=puabo-ott-tv-streaming:* npm start
```
