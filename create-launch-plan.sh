#!/usr/bin/env bash
# N3XUS v-COS – Jan 19 Launch Execution Plan Helper
# Paste this into GitHub Codespaces terminal at the repo root.

set -euo pipefail

REPO_ROOT="$(pwd)"
PLAN_FILE="$REPO_ROOT/LAUNCH_DAY_EXECUTION_PLAN.md"
LAUNCH_DATE="2026-01-19"

echo ""
echo "============================================================"
echo " N3XUS v-COS – Launch Execution Plan Scaffolder"
echo " Target Launch Date: $LAUNCH_DATE"
echo " Repo: $REPO_ROOT"
echo "============================================================"
echo ""
...
EOF
  ...
  echo "Created execution plan Markdown at:"
  echo "  $PLAN_FILE"
  echo ""
}

print_terminal_plan_summary() {
  echo "---------------- LAUNCH DAY EXECUTION PLAN (TERMINAL SUMMARY) ----------------"
  echo ""
  echo "0) Before Jan 19 (Now–18th):"
  echo "   - Keep VPS on Canonical 13 + N3XUS LAW only."
  echo "   - Finish Phase 2 / 2.5 / 3 wiring in Codespaces/staging with feature flags."
  echo ""
  echo "1) Block 1 (08:00–11:00) – Preflight & GO/NO-GO:"
  echo "   - Check docker, free -h, uptime."
  echo "   - Verify LAW handshake (blocked vs 200)."
  echo "   - Run TRAE/canon-verifier and Phase 4 prechecks."
  echo ""
  echo "2) Block 2 (11:00–14:00) – Controlled Phase 2.5 + 3:"
  echo "   - Enable OTT (Phase 2.5) for internal/test users."
  echo "   - Enable N3X-UP (Phase 3) internal-only."
  echo "   - Monitor health, logs, and load closely."
  echo ""
  echo "3) Block 3 (14:00–17:00) – Phase 4 Global Launch:"
  echo "   - Run deploy-phase-4-full-launch.sh on VPS."
  echo "   - Confirm audit log + rollback script."
  echo "   - Widen feature flags for marketplace, OTT, N3X-UP."
  echo ""
  echo "4) Block 4 (17:00–End) – Live Monitoring:"
  echo "   - Re-run canon-verifier on the live Phase 4 stack."
  echo "   - Watch docker, system metrics, and LAW endpoints."
  echo "   - Keep rollback script and feature flag controls ready."
  echo ""
  echo "Markdown plan is in LAUNCH_DAY_EXECUTION_PLAN.md (ready for PR)."
  echo "-------------------------------------------------------------------------------"
  echo ""
}
...
echo "  1) Open LAUNCH_DAY_EXECUTION_PLAN.md in Codespaces and review/edit."
echo "  2) git add / commit / push it as part of your launch PR."
echo ""
