...
app.get('/health', (req, res) => {
  res.json({ status: 'healthy', service: 'load-balancer-config' });
});

app.get('/', (req, res) => {
  res.json({
    service: 'load-balancer-config',
    role: 'Load Balancer Config',
...
app.listen(PORT, () => {
  console.log(`✅ load-balancer-config listening on port ${PORT}`);
});
